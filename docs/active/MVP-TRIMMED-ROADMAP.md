...
### G8: Leads API
**Hedef:** `/leads` ve `/lead/{domain}` endpoint'leri çalışıyor, filtreleme yapıyor.

**İşler:**
- `app/api/leads.py` (`GET /leads`, `GET /lead/{domain}`)
...
- `GET /lead/example.com` → single lead details (signals + score + reason)
- Empty result → `[]` (200 OK)
- Invalid domain → 404

**Done Criteria:**
- ✅ `/leads` filtreleme çalışıyor
- ✅ `/lead/{domain}` single lead dönüyor
- ✅ Response time <1s
