...
class EmailGenerateResponse(BaseModel):
    ...
    emails: List[str] = Field(..., description="List of generated generic email addresses")


class EmailGenerateAndValidateRequest(BaseModel):
    """Request model for email generation and validation."""
    domain: str = Field(..., description="Domain name (will be normalized)")
    use_smtp: bool = Field(False, description="If True, perform SMTP validation (default: False)")


class EmailCheck(BaseModel):
    """Email validation result model."""
    email: str = Field(..., description="Email address")
    status: str = Field(..., description="Validation status: 'valid', 'invalid', or 'unknown'")
...
class EmailGenerateAndValidateResponse(BaseModel):
    ...
    domain: str = Field(..., description="Normalized domain name")
    emails: List[EmailCheck] = Field(..., description="List of generated emails with validation results")
...
@router.post("/generate-and-validate", response_model=EmailGenerateAndValidateResponse, status_code=200)
