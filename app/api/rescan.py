...
async def rescan_single_domain(domain: str, db: Session = Depends(get_db)):
    """
...
async def bulk_rescan(
    ...
    if len(domains) > MAX_BULK_SCAN_DOMAINS:
        raise HTTPException(
            status_code=400,
            detail=f"Maximum {MAX_BULK_SCAN_DOMAINS} domains per bulk rescan",
        )
    ...
    if not normalized_domains:
        raise HTTPException(
            status_code=400, detail="No valid domains after normalization"
        )
