...
class TestErrorHandling:
    ...
    def test_scan_single_domain_dns_error(self, db_session):
        ...
        # Create company
        company = Company(
            domain="test.com", company_name="Test Inc", email="test@test.com"
        )
        ...
        with patch("app.core.tasks.analyze_dns") as mock_dns:
            ...
            assert (
                "dns" in result["error"].lower() or "failed" in result["error"].lower()
            )
    ...
    def test_scan_single_domain_whois_error(self, db_session):
        ...
        with patch("app.core.tasks.analyze_dns") as mock_dns, patch(
            "app.core.tasks.get_whois_info"
        ) as mock_whois:
    ...
    def test_scan_single_domain_database_error(self, db_session):
        ...
        # Mock database error
        with patch("app.core.tasks.analyze_dns") as mock_dns, patch.object(
            db_session, "commit"
        ) as mock_commit:
    ...
    def test_scan_single_domain_partial_failure_continues(self, db_session):
        ...
        companies = [
            Company(
                domain="valid1.com", company_name="Valid 1", email="test@valid1.com"
            ),
            Company(
                domain="valid2.com", company_name="Valid 2", email="test@valid2.com"
            ),
            Company(
                domain="invalid.com", company_name="Invalid", email="test@invalid.com"
            ),
        ]
