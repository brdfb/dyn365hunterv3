...
def test_company(db: Session):
    ...
    company = Company(canonical_name="Test Company", domain="test-tasks.com")
    db.add(company)
    db.commit()
    return company
...
class TestBulkScanTask:
    ...
    def test_bulk_scan_task_with_rescan(
        self, db: Session, test_company, test_domain_with_signal
    ):
    ...
    def test_bulk_scan_task_domain_list_not_found(self, db: Session):
        ...
        tracker.redis_client.setex(job_key, 3600, json.dumps(job_data))
...
class TestDailyRescanTask:
    ...
    def test_daily_rescan_task_with_domains(
        self, db: Session, test_company, test_domain_with_signal
    ):
...
class TestScanSingleDomain:
    ...
    def test_scan_single_domain_dns_error(self, db: Session, test_company):
        ...
        with patch("app.core.tasks.analyze_dns") as mock_dns:
            mock_dns.return_value = {"status": "dns_timeout", "error": "DNS timeout"}
