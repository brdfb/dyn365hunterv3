...
def upgrade() -> None:
    ...
    # Check if alert_config table exists, create if not
    from sqlalchemy import inspect
    conn = op.get_bind()
    inspector = inspect(conn)
    tables = inspector.get_table_names()
    
    if 'alert_config' not in tables:
        # Create alert_config table if it doesn't exist
        op.create_table(
            'alert_config',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('user_id', sa.String(length=255), nullable=False, server_default="'default'::character varying"),
            sa.Column('alert_type', sa.String(length=50), nullable=False),
            sa.Column('notification_method', sa.String(length=50), nullable=False),
            sa.Column('enabled', sa.Boolean(), nullable=False, server_default=sa.text('true')),
            sa.Column('frequency', sa.String(length=50), nullable=False, server_default="'immediate'::character varying"),
        ...
        # Table exists, perform alter operations
    ...
    if 'alerts' not in tables:
        ...
        op.create_table(
            ...
            sa.Column('status', sa.String(length=50), nullable=False, server_default="'pending'::character varying"),
