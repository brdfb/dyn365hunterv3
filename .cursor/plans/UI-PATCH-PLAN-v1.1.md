...
- [ ] **1.1** Skor detay modal - DKIM birleştirme (UI)
  - [ ] `mini-ui/js/ui-leads.js` - `showScoreBreakdown()` fonksiyonunu güncelle
  - [ ] `getRiskLabel()` helper fonksiyonu ekle
  - [ ] Test: DKIM eksik domain'de tek satır görünmeli

- [ ] **1.2** Skor detay modal - DMARC_NONE kategorisi düzeltme
  - [ ] `mini-ui/js/ui-leads.js` - `signal_points` filtreleme ekle
  - [ ] `getSignalLabel()` helper fonksiyonu ekle
  - [ ] Test: DMARC_NONE "Pozitif Sinyaller" bölümünde görünmemeli

- [ ] **1.3** Provider renkli badge'ler
  - [ ] `mini-ui/styles.css` - Provider badge CSS'leri ekle
  - [ ] `mini-ui/js/ui-leads.js` - `getProviderBadgeClass()` helper ekle
  - [ ] `renderLeadsTable()` fonksiyonunu güncelle
  - [ ] Test: Her provider farklı renkli badge ile görünmeli

### P2 - Minor UI İyileştirmeleri

- [ ] **2.1** Sort ikonları - Tooltip + daha belirgin
  - [ ] `mini-ui/styles.css` - İkon boyutunu artır
  - [ ] `mini-ui/index.html` - Tooltip attribute'ları ekle
  - [ ] Test: Hover yapınca tooltip görünmeli, ikonlar daha belirgin

- [ ] **2.2** CSV feedback - Toast notification (Opsiyonel)
